...
_dayjs.default.extend(function (_o, c) {
...
function findTargetStr(val, index, segmentation) {
  var items = (0, _toConsumableArray2.default)(new Set(val.split(segmentation)));
  var idx = 0;
...
var toDateWithValueFormat = function toDateWithValueFormat(val, valueFormat) {
  ...
  var baseDate = (0, _dayjs.default)(val, valueFormat);
  if (matchs === null) {
    return baseDate;
  }
  var _iterator = (0, _createForOfIteratorHelper2.default)(matchs),
    _step;
  try {
...
var generateConfig = {
  ...
  toString: function toString(value, valueFormat) {
    if (Array.isArray(value)) {
      return value.map(function (val) {
        return _dayjs.default.isDayjs(val) ? val.format(valueFormat) : val;
      });
    } else {
